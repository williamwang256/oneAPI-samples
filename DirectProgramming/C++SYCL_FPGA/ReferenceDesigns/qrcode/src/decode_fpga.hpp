#include <sycl/ext/intel/ac_types/ac_int.hpp>
#include <sycl/ext/intel/fpga_extensions.hpp>
#include <sycl/sycl.hpp>

#include "quirc.hpp"
#include "version_db.hpp"

#ifdef __SYCL_DEVICE_ONLY__
  #define CL_CONSTANT __attribute__((opencl_constant))
#else
  #define CL_CONSTANT
#endif
#define PRINTF(format, ...) { \
      static const CL_CONSTANT char _format[] = format; \
      sycl::ext::oneapi::experimental::printf(_format, ## __VA_ARGS__); }

#define MAX_POLY 64

/************************************************************************
 * Galois fields
 */

struct galois_field {
  int p;
  const uint8_t *log;
  const uint8_t *exp;
};

constexpr uint8_t gf16_exp[16] = {
    0x01, 0x02, 0x04, 0x08, 0x03, 0x06, 0x0c, 0x0b, 0x05, 0x0a, 0x07, 0x0e,
    0x0f, 0x0d, 0x09, 0x01};

constexpr uint8_t gf16_log[16] = {
    0x00, 0x0f, 0x01, 0x04, 0x02, 0x08, 0x05, 0x0a, 0x03, 0x0e, 0x09, 0x07,
    0x06, 0x0d, 0x0b, 0x0c};

constexpr struct galois_field gf16 = {
    .p = 15, .log = gf16_log, .exp = gf16_exp};

constexpr uint8_t gf256_exp[256] = {
    };

constexpr uint8_t gf256_log[256] = {
    };

constexpr struct galois_field gf256 = {
    .p = 255, .log = gf256_log, .exp = gf256_exp};

/************************************************************************
 * Polynomial operations
 */
template<struct galois_field gf>
void poly_add(uint8_t *dst, const uint8_t *src, uint8_t c, int shift, bool dummy) {
  int log_c = gf.log[c];

  if (!c) {
    return;
  }

  for (int i = 0; i < MAX_POLY; i++) {
    int p = i + shift;
    uint8_t v = src[i];
    if (p < 0 || p >= MAX_POLY) {
      continue;
    }
    if (!v) {
      continue;
    }
    dst[p] = dummy ? dst[p] : dst[p] ^ gf.exp[(gf.log[v] + log_c) % gf.p];
  }
}

template<struct galois_field gf>
uint8_t poly_eval(const uint8_t *s, uint8_t x) {
  uint8_t sum = 0;
  uint8_t log_x = gf.log[x];

  if (!x) {
    return s[0];
  }

  for (int i = 0; i < MAX_POLY; i++) {
    uint8_t c = s[i];
    if (!c) {
      continue;
    }
    sum ^= gf.exp[(gf.log[c] + log_x * i) % gf.p];
  }

  return sum;
}

/************************************************************************
 * Berlekamp-Massey algorithm for finding error locator polynomials.
 */

template<struct galois_field gf>
void berlekamp_massey(const uint8_t *s, int N, uint8_t *sigma) {
  uint8_t C[MAX_POLY];
  uint8_t B[MAX_POLY];
  uint8_t T[MAX_POLY];
  int L = 0;
  int m = 1;
  uint8_t b = 1;

  #pragma unroll
  for (int k = 0; k < MAX_POLY; k++) {
    B[k] = 0;
    C[k] = 0;
  }
  B[0] = 1;
  C[0] = 1;

  for (int n = 0; n < N; n++) {
    uint8_t d = s[n];
    uint8_t mult;
    
    for (int i = 1; i <= L; i++) {
      if ((C[i] && s[n - i])) {
        // continue;
        d ^= gf.exp[(gf.log[C[i]] + gf.log[s[n - i]]) % gf.p];
      }
      // d ^= gf.exp[(gf.log[C[i]] + gf.log[s[n - i]]) % gf.p];
    }

    mult = gf.exp[(gf.p - gf.log[b] + gf.log[d]) % gf.p];
    
    bool dummy;
    
    if (!d) {
      m++;
      dummy = true;
    } else if (L * 2 <= n) {
      #pragma unroll
      for (int k = 0; k < MAX_POLY; k++) {
        T[k] = C[k];
      }
      dummy = false;
      #pragma unroll
      for (int k = 0; k < MAX_POLY; k++) {
        B[k] = T[k];
      }
      L = n + 1 - L;
      b = d;
      m = 1;
    } else {
      dummy = false;
      m++;
    }
    poly_add<gf>(C, B, mult, m, dummy);
  }

  #pragma unroll
  for (int k = 0; k < MAX_POLY; k++) {
    sigma[k] = C[k];
  }
}

/************************************************************************
 * Code stream error correction
 *
 * Generator polynomial for GF(2^8) is x^8 + x^4 + x^3 + x^2 + 1
 */

int block_syndromes(const uint8_t *data, int bs, int npar, uint8_t *s) {
  int nonzero = 0;
  memset(s, 0, MAX_POLY);

  for (int i = 0; i < npar; i++) {
    for (int j = 0; j < bs; j++) {
      uint8_t c = data[bs - j - 1];
      if (!c) {
        continue;
      }
      s[i] ^= gf256_exp[((int)gf256_log[c] + i * j) % 255];
    }
    if (s[i]) {
      nonzero = 1;
    }
  }

  return nonzero;
}

void eloc_poly(uint8_t *omega, const uint8_t *s, const uint8_t *sigma, int npar) {
  memset(omega, 0, MAX_POLY);

  for (int i = 0; i < npar; i++) {
    const uint8_t a = sigma[i];
    const uint8_t log_a = gf256_log[a];
    if (!a) {
      continue;
    }
    for (int j = 0; j + 1 < MAX_POLY; j++) {
      const uint8_t b = s[j + 1];
      if (i + j >= npar) {
        break;
      }
      if (!b) {
        continue;
      }
      omega[i + j] ^= gf256_exp[(log_a + gf256_log[b]) % 255];
    }
  }
}

quirc_decode_error_t correct_block(uint8_t *data, struct quirc_rs_params *ecc) {
  int npar = ecc->bs - ecc->dw;
  [[intel::fpga_memory]] uint8_t s[MAX_POLY];
  uint8_t sigma[MAX_POLY];
  uint8_t sigma_deriv[MAX_POLY];
  uint8_t omega[MAX_POLY];

  /* Compute syndrome vector */
  if (!block_syndromes(data, ecc->bs, npar, s))
    return QUIRC_SUCCESS;

  berlekamp_massey<gf256>(s, npar, sigma);

  /* Compute derivative of sigma */
  memset(sigma_deriv, 0, MAX_POLY);
  for (int i = 0; i < MAX_POLY - 1; i += 2) {
    sigma_deriv[i] = sigma[i + 1];
  }

  /* Compute error evaluator polynomial */
  eloc_poly(omega, s, sigma, npar - 1);

  /* Find error locations and magnitudes */
  for (int i = 0; i < ecc->bs; i++) {
    uint8_t xinv = gf256_exp[255 - i];

    if (!poly_eval<gf256>(sigma, xinv)) {
      uint8_t sd_x = poly_eval<gf256>(sigma_deriv, xinv);
      uint8_t omega_x = poly_eval<gf256>(omega, xinv);
      uint8_t error = gf256_exp[(255 - gf256_log[sd_x] + gf256_log[omega_x]) % 255];
      data[ecc->bs - i - 1] ^= error;
    }
  }

  if (block_syndromes(data, ecc->bs, npar, s)) {
    return QUIRC_ERROR_DATA_ECC;
  }

  return QUIRC_SUCCESS;
}

/************************************************************************
 * Format value error correction
 *
 * Generator polynomial for GF(2^4) is x^4 + x + 1
 */

#define FORMAT_MAX_ERROR 3
#define FORMAT_SYNDROMES (FORMAT_MAX_ERROR * 2)
#define FORMAT_BITS 15

int format_syndromes(uint16_t u, uint8_t *s) {
  int nonzero = 0;

  #pragma unroll
  for (int i = 0; i < MAX_POLY; i++) {
    s[i] = 0;
  }

  [[intel::loop_coalesce]]
  for (int i = 0; i < FORMAT_SYNDROMES; i++) {
    // s[i] = 0;
    for (int j = 0; j < FORMAT_BITS; j++)
      if (u & (1 << j))
        s[i] ^= gf16_exp[((i + 1) * j) % 15];

    if (s[i])
      nonzero = 1;
  }

  return nonzero;
}

quirc_decode_error_t correct_format(uint16_t *f_ret) {
  uint16_t u = *f_ret;
  uint8_t s[MAX_POLY];
  uint8_t sigma[MAX_POLY];

  /* Evaluate U (received codeword) at each of alpha_1 .. alpha_6
   * to get S_1 .. S_6 (but we index them from 0).
   */
  if (!format_syndromes(u, s)) {
    return QUIRC_SUCCESS;
  }

  berlekamp_massey<gf16>(s, FORMAT_SYNDROMES, sigma);

  /* Now, find the roots of the polynomial */
  for (int i = 0; i < 15; i++) {
    if (!poly_eval<gf16>(sigma, gf16_exp[15 - i])) {
      u ^= (1 << i);
    }
  }

  if (format_syndromes(u, s)) {
    return QUIRC_ERROR_FORMAT_ECC;
  }

  *f_ret = u;
  return QUIRC_SUCCESS;
}

/************************************************************************
 * Decoder algorithm
 */

struct datastream {
  uint8_t *raw;
  int data_bits;
  int ptr;

  uint8_t data[QUIRC_MAX_PAYLOAD];
};

 inline int grid_bit(int size, uint8_t *cell_bitmap, int x, int y) {
  int p = y * size + x;
  return (cell_bitmap[p >> 3] >> (p & 7)) & 1;
}

 quirc_decode_error_t read_format(int size, uint8_t *cell_bitmap, int &ecc_level, int &mask, int which) {
  uint16_t format = 0;
  uint16_t fdata;
  quirc_decode_error_t err;

  if (which) {
    for (int i = 0; i < 7; i++) {
      format = (format << 1) | grid_bit(size, cell_bitmap, 8, size - 1 - i);
    }
    for (int i = 0; i < 8; i++) {
      format = (format << 1) | grid_bit(size, cell_bitmap, size - 8 + i, 8);
    }
  } else {
     const int xs[15] = {8, 8, 8, 8, 8, 8, 8, 8, 7, 5, 4, 3, 2, 1, 0};
     const int ys[15] = {0, 1, 2, 3, 4, 5, 7, 8, 8, 8, 8, 8, 8, 8, 8};

    for (int i = 14; i >= 0; i--) {
      format = (format << 1) | grid_bit(size, cell_bitmap, xs[i], ys[i]);
    }
  }

  format ^= 0x5412;
  err = correct_format(&format);
  if (err) {
    return err;
  }

  fdata = format >> 10;
  ecc_level = fdata >> 3;
  mask = fdata & 7;

  return QUIRC_SUCCESS;
}

int mask_bit(int mask, int i, int j) {
  switch (mask) {
    case 0:
      return !((i + j) % 2);
    case 1:
      return !(i % 2);
    case 2:
      return !(j % 3);
    case 3:
      return !((i + j) % 3);
    case 4:
      return !(((i / 2) + (j / 3)) % 2);
    case 5:
      return !((i * j) % 2 + (i * j) % 3);
    case 6:
      return !(((i * j) % 2 + (i * j) % 3) % 2);
    case 7:
      return !(((i * j) % 3 + (i + j) % 2) % 2);
  }
  return 0;
}

template<int version>
int reserved_cell(int i, int j) {
  constexpr struct quirc_version_info ver = quirc_version_db[version];
  constexpr int size = version * 4 + 17;
  int ai = -1, aj = -1, a;

  /* Finder + format: top left */
  if (i < 9 && j < 9) {
    return 1;
  }

  /* Finder + format: bottom left */
  if (i + 8 >= size && j < 9) {
    return 1;
  }

  /* Finder + format: top right */
  if (i < 9 && j + 8 >= size) {
    return 1;
  }

  /* Exclude timing patterns */
  if (i == 6 || j == 6) {
    return 1;
  }

  /* Exclude version info, if it exists. Version info sits adjacent to
   * the top-right and bottom-left finders in three rows, bounded by
   * the timing pattern.
   */
  if (version >= 7) {
    if (i < 6 && j + 11 >= size) {
      return 1;
    }
    if (i + 11 >= size && j < 6) {
      return 1;
    }
  }

  /* Exclude alignment patterns */
  for (a = 0; a < QUIRC_MAX_ALIGNMENT && ver.apat[a]; a++) {
    int p = ver.apat[a];
    if (abs(p - i) < 3) {
      ai = a;
    }
    if (abs(p - j) < 3) {
      aj = a;
    }
  }

  if (ai >= 0 && aj >= 0) {
    a--;
    if (ai > 0 && ai < a) {
      return 1;
    }
    if (aj > 0 && aj < a) {
      return 1;
    }
    if (aj == a && ai == a) {
      return 1;
    }
  }

  return 0;
}

void read_bit(int size, uint8_t *cell_bitmap, int mask, struct datastream *ds, int i, int j) {
  int bitpos = ds->data_bits & 7;
  int bytepos = ds->data_bits >> 3;
  int v = grid_bit(size, cell_bitmap, j, i);
  if (mask_bit(mask, i, j)) {
    v ^= 1;
  }
  if (v) {
    ds->raw[bytepos] |= (0x80 >> bitpos);
  }
  ds->data_bits++;
}

template<int version, int size>
void read_data(uint8_t *cell_bitmap, int mask, struct datastream *ds) {
  int y = size - 1;
  int x = size - 1;
  int dir = -1;

  while (x > 0) {
    if (x == 6) {
      x--;
    }
    if (!reserved_cell<version>(y, x)) {
      read_bit(size, cell_bitmap, mask, ds, y, x);
    }
    if (!reserved_cell<version>(y, x - 1)) {
      read_bit(size, cell_bitmap, mask, ds, y, x - 1);
    }
    y += dir;
    if (y < 0 || y >= size) {
      dir = -dir;
      x -= 2;
      y += dir;
    }
  }
}

template<int version>
quirc_decode_error_t codestream_ecc(int ecc_level, struct datastream *ds) {
  constexpr struct quirc_version_info ver = quirc_version_db[version];
  struct quirc_rs_params sb_ecc = ver.ecc[ecc_level];
  struct quirc_rs_params lb_ecc;
  const int lb_count = (ver.data_bytes - sb_ecc.bs * sb_ecc.ns) / (sb_ecc.bs + 1);
  const int bc = lb_count + sb_ecc.ns;
  const int ecc_offset = sb_ecc.dw * bc + lb_count;
  int dst_offset = 0;

  memcpy(&lb_ecc, &sb_ecc, sizeof(lb_ecc));
  lb_ecc.dw++;
  lb_ecc.bs++;

  for (int i = 0; i < bc; i++) {
    uint8_t *dst = ds->data + dst_offset;
    struct quirc_rs_params *ecc = (i < sb_ecc.ns) ? &sb_ecc : &lb_ecc;
    const int num_ec = ecc->bs - ecc->dw;
    quirc_decode_error_t err;
    for (int j = 0; j < ecc->dw; j++) {
      dst[j] = ds->raw[j * bc + i];
    }
    for (int j = 0; j < num_ec; j++) {
      dst[ecc->dw + j] = ds->raw[ecc_offset + j * bc + i];
    }
    err = correct_block(dst, ecc);
    if (err) {
      return err;
    }
    dst_offset += ecc->dw;
  }
  ds->data_bits = dst_offset * 8;
  return QUIRC_SUCCESS;
}

inline int bits_remaining(struct datastream *ds) {
  return ds->data_bits - ds->ptr;
}

int take_bits(struct datastream *ds, int len) {
  int ret = 0;
  while (len && (ds->ptr < ds->data_bits)) {
    uint8_t b = ds->data[ds->ptr >> 3];
    int bitpos = ds->ptr & 7;
    ret <<= 1;
    if ((b << bitpos) & 0x80) {
      ret |= 1;
    }
    ds->ptr++;
    len--;
  }
  return ret;
}

int numeric_tuple(uint8_t *payload, int &payload_len, struct datastream *ds, int bits, int digits) {
  int tuple;
  if (bits_remaining(ds) < bits) {
    return -1;
  }
  tuple = take_bits(ds, bits);
  for (int i = digits - 1; i >= 0; i--) {
    payload[payload_len + i] = tuple % 10 + '0';
    tuple /= 10;
  }
  payload_len += digits;
  return 0;
}

template<int version>
quirc_decode_error_t decode_numeric(uint8_t *payload, int &payload_len, struct datastream *ds) {
  int bits = 14;
  int count;

  if (version < 10) {
    bits = 10;
  } else if (version < 27) {
    bits = 12;
  }

  count = take_bits(ds, bits);
  if (payload_len + count + 1 > QUIRC_MAX_PAYLOAD){
    return QUIRC_ERROR_DATA_OVERFLOW;
  }

  while (count >= 3) {
    if (numeric_tuple(payload, payload_len, ds, 10, 3) < 0) {
      return QUIRC_ERROR_DATA_UNDERFLOW;
    }
    count -= 3;
  }

  if (count >= 2) {
    if (numeric_tuple(payload, payload_len, ds, 7, 2) < 0) {
      return QUIRC_ERROR_DATA_UNDERFLOW;
    }
    count -= 2;
  }

  if (count) {
    if (numeric_tuple(payload, payload_len, ds, 4, 1) < 0) {
      return QUIRC_ERROR_DATA_UNDERFLOW;
    }
    count--;
  }

  return QUIRC_SUCCESS;
}

int alpha_tuple(uint8_t *payload, int &payload_len, struct datastream *ds, int bits, int digits) {
  int tuple;
  if (bits_remaining(ds) < bits) {
    return -1;
  }
  tuple = take_bits(ds, bits);
  for (int i = 0; i < digits; i++) {
    const char *alpha_map = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ $%*+-./:";
    payload[payload_len + digits - i - 1] = alpha_map[tuple % 45];
    tuple /= 45;
  }
  payload_len += digits;
  return 0;
}

template<int version>
quirc_decode_error_t decode_alpha(uint8_t *payload, int &payload_len, struct datastream *ds) {
  int bits = 13;
  int count;
  if (version < 10) {
    bits = 9;
  } else if (version < 27) {
    bits = 11;
  }
  count = take_bits(ds, bits);
  if (payload_len + count + 1 > QUIRC_MAX_PAYLOAD) {
    return QUIRC_ERROR_DATA_OVERFLOW;
  }
  while (count >= 2) {
    if (alpha_tuple(payload, payload_len, ds, 11, 2) < 0) {
      return QUIRC_ERROR_DATA_UNDERFLOW;
    }
    count -= 2;
  }
  if (count) {
    if (alpha_tuple(payload, payload_len, ds, 6, 1) < 0) {
      return QUIRC_ERROR_DATA_UNDERFLOW;
    }
    count--;
  }
  return QUIRC_SUCCESS;
}

template<int version>
quirc_decode_error_t decode_byte(uint8_t *payload, int &payload_len, struct datastream *ds) {
  int bits = 16;
  int count;
  if (version < 10) {
    bits = 8;
  }
  count = take_bits(ds, bits);
  if (payload_len + count + 1 > QUIRC_MAX_PAYLOAD) {
    return QUIRC_ERROR_DATA_OVERFLOW;
  }
  if (bits_remaining(ds) < count * 8) {
    return QUIRC_ERROR_DATA_UNDERFLOW;
  }
  for (int i = 0; i < count; i++) {
    payload[payload_len++] = take_bits(ds, 8);
  }
  return QUIRC_SUCCESS;
}

template<int version>
quirc_decode_error_t decode_kanji(uint8_t *payload, int &payload_len, struct datastream *ds) {
  int bits = 12;
  int count;
  if (version < 10) {
    bits = 8;
  } else if (version < 27) {
    bits = 10;
  }
  count = take_bits(ds, bits);
  if (payload_len + count * 2 + 1 > QUIRC_MAX_PAYLOAD) {
    return QUIRC_ERROR_DATA_OVERFLOW;
  }
  if (bits_remaining(ds) < count * 13) {
    return QUIRC_ERROR_DATA_UNDERFLOW;
  }
  for (int i = 0; i < count; i++) {
    int d = take_bits(ds, 13);
    int msB = d / 0xc0;
    int lsB = d % 0xc0;
    int intermediate = (msB << 8) | lsB;
    uint16_t sjw;
    if (intermediate + 0x8140 <= 0x9ffc) {
      /* bytes are in the range 0x8140 to 0x9FFC */
      sjw = intermediate + 0x8140;
    } else {
      /* bytes are in the range 0xE040 to 0xEBBF */
      sjw = intermediate + 0xc140;
    }
    payload[payload_len++] = sjw >> 8;
    payload[payload_len++] = sjw & 0xff;
  }
  return QUIRC_SUCCESS;
}

quirc_decode_error_t decode_eci(uint32_t &eci, struct datastream *ds) {
  if (bits_remaining(ds) < 8) {
    return QUIRC_ERROR_DATA_UNDERFLOW;
  }
  eci = take_bits(ds, 8);
  if ((eci & 0xc0) == 0x80) {
    if (bits_remaining(ds) < 8) {
      return QUIRC_ERROR_DATA_UNDERFLOW;
    }
    eci = (eci << 8) | take_bits(ds, 8);
  } else if ((eci & 0xe0) == 0xc0) {
    if (bits_remaining(ds) < 16) {
      return QUIRC_ERROR_DATA_UNDERFLOW;
    }
    eci = (eci << 16) | take_bits(ds, 16);
  }
  return QUIRC_SUCCESS;
}

template<int version>
quirc_decode_error_t decode_payload(int &data_type, uint8_t *payload, int &payload_len, uint32_t &eci, struct datastream *ds) {
  while (bits_remaining(ds) >= 4) {
    quirc_decode_error_t err = QUIRC_SUCCESS;
    int type = take_bits(ds, 4);

    switch (type) {
      case QUIRC_DATA_TYPE_NUMERIC:
        err = decode_numeric<version>(payload, payload_len, ds);
        break;

      case QUIRC_DATA_TYPE_ALPHA:
        err = decode_alpha<version>(payload, payload_len, ds);
        break;

      case QUIRC_DATA_TYPE_BYTE:
        err = decode_byte<version>(payload, payload_len, ds);
        break;

      case QUIRC_DATA_TYPE_KANJI:
        err = decode_kanji<version>(payload, payload_len, ds);
        break;

      case 7:
        err = decode_eci(eci, ds);
        break;

      default:
        /* Add nul terminator to all payloads */
        if (payload_len >= (int)(QUIRC_MAX_PAYLOAD * sizeof(uint8_t))) {
          payload_len--;
        }
        payload[payload_len] = 0;
        return QUIRC_SUCCESS;
    }
    if (err) {
      return err;
    }
    if (!(type & (type - 1)) && (type > data_type)) {
      data_type = type;
    }
  }
  /* Add nul terminator to all payloads */
  if (payload_len >= (int)(QUIRC_MAX_PAYLOAD * sizeof(uint8_t))) {
    payload_len--;
  }
  payload[payload_len] = 0;
  return QUIRC_SUCCESS;
}
