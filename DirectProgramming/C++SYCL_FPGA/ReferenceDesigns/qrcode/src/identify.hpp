...
static int line_intersect(struct quirc_point *p0,
                          struct quirc_point *p1,
                          struct quirc_point *q0,
                          struct quirc_point *q1, struct quirc_point *r) {
...
static void perspective_setup(float *c, struct quirc_point *rect,
                              float w, float h) {
  float x0 = rect[0].x;
  float y0 = rect[0].y;
  float x1 = rect[1].x;
...
static void perspective_map(const float *c, float u,
                            float v, struct quirc_point *ret) {
...
static void perspective_unmap(const float *c,
                              struct quirc_point *in, float *u,
                              float *v) {
  float x = in->x;
  float y = in->y;
  float den = -c[0] * c[7] * y + c[1] * c[6] * y +
                      (c[3] * c[7] - c[4] * c[6]) * x + c[0] * c[4] -
                      c[1] * c[3];
...
static void flood_fill_line(struct quirc *q, int x, int y, int from, int to,
                            span_func_t func, void *user_data, int *leftp,
                            int *rightp) {
  uint8_t *row;
  int left;
  int right;
...
static struct quirc_flood_fill_vars *
flood_fill_call_next(struct quirc *q, uint8_t *row, int from, int to,
  ...
  while (*leftp <= vars->right) {
    if (row[*leftp] == from) {
      ...
      /* Fill the extent */
      flood_fill_line(q, *leftp, next_vars->y, from, to, func, user_data,
...
static void flood_fill_seed(struct quirc *q, int x0, int y0, int from, int to,
                            span_func_t func, void *user_data) {
  struct quirc_flood_fill_vars *const stack = q->flood_fill_vars;
  const size_t stack_size = q->num_flood_fill_vars;
  struct quirc_flood_fill_vars *const last_vars = &stack[stack_size - 1];
...
static uint8_t otsu(struct quirc *q) {
 ...
 * out of image bounds.
 */
static int read_cell(struct quirc *q, int index, int x, int y) {
  struct quirc_grid *qr = &q->grids[index];
...
static int fitness_cell(struct quirc *q, int index, int x, int y) {
  struct quirc_grid *qr = &q->grids[index];
  int score = 0;
  int u, v;
...
static int fitness_ring(struct quirc *q, int index, int cx, int cy,
                        int radius) {
  int i;
  int score = 0;
...
static int fitness_apat(struct quirc *q, int index, int cx, int cy) {
  return fitness_cell(q, index, cx, cy) - fitness_ring(q, index, cx, cy, 1) +
         fitness_ring(q, index, cx, cy, 2);
}

static int fitness_capstone(struct quirc *q, int index, int x, int y) {
  x += 3;
  y += 3;
 ...
 * grid.
 */
static int fitness_all(struct quirc *q, int index) {
  struct quirc_grid *qr = &q->grids[index];
...
static void rotate_capstone(struct quirc_capstone *cap,
                            struct quirc_point *h0,
                            struct quirc_point *hd) {
...
static void record_qr_grid(struct quirc *q, int a, int b, int c) {
  ...
  /* Check the timing pattern by measuring grid size. This doesn't require a
  ...
  if (qr->grid_size > 21) {
    ...
    if (qr->align_region >= 0) {
      ...
      flood_fill_seed(q, reg->seed.x, reg->seed.y, qr->align_region,
                      QUIRC_PIXEL_BLACK, NULL, NULL);
      flood_fill_seed(q, reg->seed.x, reg->seed.y, QUIRC_PIXEL_BLACK,
                      qr->align_region, find_leftmost_to_line, &psd);
    }
  }
...
static void test_neighbours(struct quirc *q, int i,
  ...
  for (int j = 0; j < hlist->count; j++) {
    struct neighbour *hn = &hlist->n[j];
    for (int k = 0; k < vlist->count; k++) {
      struct neighbour *vn = &vlist->n[k];
      float squareness = fabs(1.0 - hn->distance / vn->distance);
      if (squareness < 0.2)
...
static void pixels_setup(struct quirc *q, uint8_t threshold) {
  if (QUIRC_PIXEL_ALIAS_IMAGE) {
    q->pixels = (uint8_t *)q->image;
  }

  uint8_t *source = q->image;
  uint8_t *dest = q->pixels;
  for (int i = 0; i < q->w * q->h; i++) {
    dest[i] = (source[i] < threshold) ? QUIRC_PIXEL_BLACK : QUIRC_PIXEL_WHITE;
  }
}
 ...
 * 
 */
void quirc_extract(struct quirc *q, int index, struct quirc_code *code) {
  struct quirc_grid *qr = &q->grids[index];
  int y;
  int i = 0;
